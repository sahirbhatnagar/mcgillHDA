...
#'
#' This function calculates confidence intervals for lasso using bootstrap
...
#' @param alpha      Error rate; 0.05 (default) corresponds to 0.95 confidence interval
#' @param bar        Print a progress bar?
#'
#' @import glmnet gglasso ncvreg
#'
#' @export
...
boot.glmnet <- function(X, y, B=500, lambda, seed, alpha=0.05, bar = TRUE) {
  ...
  if (bar) pb <- utils::txtProgressBar(0, B, style=3)
  beta_hat <- matrix(NA, B, p, dimnames=list(1:B, colnames(X)))
  if (!missing(seed)) set.seed(seed)
  for (i in 1:B) {
    ind <- sample(1:n, replace=TRUE)
    fit.i <- glmnet::glmnet(X[ind,], y[ind], nlambda=20, lambda.min.ratio=lambda)
    b <- stats::coef(fit.i, s=lambda)
    beta_hat[i,] <- b[-1]
    if (bar) utils::setTxtProgressBar(pb, i)
  }
  if (bar) close(pb)
  out <- as.data.frame(t(apply(beta_hat, 2, stats::quantile, probs=c(alpha/2, 1-alpha/2))))
